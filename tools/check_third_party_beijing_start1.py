#!/usr/bin/env python3
"""Validate the narrow Beijing start-from-grade-1 primary source-adapter contract."""
...
PATH = ROOT / "anki" / "klose" / "source_reference" / "beijing_start1_staging" / "occurrences.csv"
EXPECTED_FIELDS = [
    "SourceOccurrenceKey", "SourceID", "SourceBook", "Grade", "Semester",
    "SourceRow", "Word", "MatchKey", "British", "American", "Definition", "SourceFile",
]
SOURCE_FILE_RE = re.compile(r"北京版一年级起点[一二三四五六]年级[上下]\.xlsx$")
EXPECTED_OCCURRENCES = 808
EXPECTED_MATCHKEYS = 734
...
def main() -> None:
    ...
    if len(rows) != EXPECTED_OCCURRENCES:
        raise SystemExit(f"Expected {EXPECTED_OCCURRENCES} Beijing start1 occurrences, found {len(rows)}")
    if any(row.get("SourceID") != "beijing_start1" for row in rows):
        raise SystemExit("SourceID drift in Beijing start1 adapter")
    ...
    if any(not SOURCE_FILE_RE.fullmatch(row.get("SourceFile", "")) for row in rows):
        raise SystemExit("Unexpected source file leaked into Beijing start1 primary adapter")
    ...
    print("Beijing Start1 Source Adapter Recheck = pass")
    ...
    print("identity/matching state in occurrence output = no")
