#!/usr/bin/env python3
"""Parse Beishida start-from-grade-3 primary vocabulary into standardized occurrences.
...
The Beishida start-from-grade-1 sequence and all middle/high-school materials in
the same raw directory are explicitly excluded. Cross-source matching,
morphology, semantic review, object routing, and corpus identity decisions belong
to tools/build_third_party_corpus.py.
...
SOURCE_DIR = ROOT / "1.全国各大教材版本中小学同步" / "北师大"
OUT = ROOT / "anki" / "klose" / "source_reference" / "beishida_start3_staging"

EXPECTED_BOOKS = [(g, s) for g in range(3, 7) for s in ("上", "下")]
GRADE_CN = {"三": 3, "四": 4, "五": 5, "六": 6}
BOOK_RE = re.compile(r"北师大版三年级起点([三四五六])年级([上下])(?:册)?\.xlsx$")
...
def parse_source(path: Path, grade: int, semester: str) -> list[dict[str, str]]:
    ...
    if len(header) < 4:
        raise SystemExit(f"Expected at least four source columns in {path.name}; header={header}")
    first_four = [norm_display(v) for v in header[:4]]
    if first_four[0] != "单词" or first_four[3] != "释义":
        raise SystemExit(f"Unexpected source schema in {path.name}; first_four={first_four}")
    ...
    for row_no, vals in rows[header_pos + 1:]:
        ...
        key = match_key(word)
        out.append({
            "SourceOccurrenceKey": f"beishida_start3|g{grade}-{sem_en}|r{row_no:03d}|{key}",
            "SourceID": "beishida_start3",
            "SourceBook": book,
            "Grade": str(grade),
            "Semester": semester,
...
def main() -> None:
    ...
    for path in SOURCE_DIR.glob("北师大版三年级起点*.xlsx"):
        ...
        if key in source_files:
            raise SystemExit(
                f"Duplicate Beishida start3 source book for {key}: "
    ...
    if missing or len(source_files) != 8:
        raise SystemExit(
            f"Beishida start3 primary source-book set invalid; missing={missing}; found={len(source_files)}"
        )
    ...
    readme = f"""# Beishida Start3 — Third-party Source Adapter

Scope:

```text
北师大版三年级起点 3-6 年级上下册 = 8 primary-school books
```

The separate 北师大版一年级起点 sequence and all middle/high-school XLSX files
...
"""
    ...
    print("Beishida start3 source adapter = valid")
    print("source books = 8")
    print(f"source occurrences = {len(occurrences)}")
    print(f"distinct MatchKeys = {distinct}")
    print("source schema = first four columns with 单词 / 释义 anchors")
