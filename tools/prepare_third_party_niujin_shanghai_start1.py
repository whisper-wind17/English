#!/usr/bin/env python3
"""Parse Oxford Shanghai Grade 1-6 primary vocabulary into standardized occurrences.
...
SOURCE_DIR = ROOT / "1.全国各大教材版本中小学同步" / "牛津版"
OUT = ROOT / "anki" / "klose" / "source_reference" / "niujin_shanghai_start1_staging"

EXPECTED_BOOKS = [(g, s) for g in range(1, 7) for s in ("上", "下")]
GRADE_CN = {"一": 1, "二": 2, "三": 3, "四": 4, "五": 5, "六": 6}
BOOK_RE = re.compile(r"牛津上海版([一二三四五六])年级([上下])册\.xlsx$")
...
def parse_source(path: Path, grade: int, semester: str) -> list[dict[str, str]]:
    ...
    for row_no, vals in rows[header_pos + 1:]:
        ...
        key = match_key(word)
        out.append({
            "SourceOccurrenceKey": f"niujin_shanghai_start1|g{grade}-{sem_en}|r{row_no:03d}|{key}",
            "SourceID": "niujin_shanghai_start1",
            "SourceBook": book,
            "Grade": str(grade),
            "Semester": semester,
...
def main() -> None:
    ...
    for path in SOURCE_DIR.glob("牛津上海版*.xlsx"):
        ...
        if key in source_files:
            raise SystemExit(
                f"Duplicate Oxford Shanghai primary source book for {key}: "
    ...
    if missing or len(source_files) != 12:
        raise SystemExit(
            f"Oxford Shanghai Grade 1-6 source-book set invalid; missing={missing}; found={len(source_files)}"
        )
    ...
    readme = f"""# Oxford Shanghai Start1 — Third-party Source Adapter
...
"""
    ...
    print("Oxford Shanghai start1 source adapter = parsed")
