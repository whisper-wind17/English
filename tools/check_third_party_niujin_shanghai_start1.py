#!/usr/bin/env python3
"""Validate the narrow Oxford Shanghai Grade 1-6 primary source-adapter contract."""
...
PATH = ROOT / "anki" / "klose" / "source_reference" / "niujin_shanghai_start1_staging" / "occurrences.csv"
EXPECTED_FIELDS = [
    "SourceOccurrenceKey", "SourceID", "SourceBook", "Grade", "Semester",
    "SourceRow", "Word", "MatchKey", "British", "American", "Definition", "SourceFile",
]
SOURCE_FILE_RE = re.compile(r"牛津上海版([一二三四五六])年级([上下])册\.xlsx$")
GRADE_CN = {"一": "1", "二": "2", "三": "3", "四": "4", "五": "5", "六": "6"}
SEM_EN = {"上": "upper", "下": "lower"}
EXPECTED_OCCURRENCES = 1186
EXPECTED_MATCHKEYS = 974
...
def main() -> None:
    ...
    if len(rows) != EXPECTED_OCCURRENCES:
        raise SystemExit(
            f"Expected {EXPECTED_OCCURRENCES} Oxford Shanghai start1 occurrences, found {len(rows)}"
        )
    if any(row.get("SourceID") != "niujin_shanghai_start1" for row in rows):
        raise SystemExit("SourceID drift in Oxford Shanghai start1 adapter")
    ...
    for row in rows:
        ...
        if not m:
            raise SystemExit(
                f"Unexpected source file leaked into Oxford Shanghai primary adapter: {row.get('SourceFile', '')}"
            )
        ...
        if row.get("SourceBook") != f"{grade}年级{semester}":
            raise SystemExit(
                f"SourceBook drift for {row.get('SourceFile')}: {row.get('SourceBook')}"
            )
        try:
        ...
        expected_prefix = f"niujin_shanghai_start1|g{grade}-{SEM_EN[semester]}|r{row_no:03d}|"
    ...
    if seen_books != expected_books:
        raise SystemExit(f"Expected 12 grade/semester books, got {sorted(seen_books)}")
    ...
    print("Oxford Shanghai Start1 Source Adapter Recheck = pass")
    ...
    print("grade-7-plus / high-school leakage = no")
