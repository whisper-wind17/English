#!/usr/bin/env python3
"""Parse Waiyan start-from-grade-1 primary vocabulary into standardized occurrences.
...
The Waiyan directory also contains the start-from-grade-3 primary family plus
middle/high-school materials. They are outside this adapter and must not leak in.
...
SOURCE_DIR = ROOT / "1.全国各大教材版本中小学同步" / "外研版"
OUT = ROOT / "anki" / "klose" / "source_reference" / "waiyan_start1_staging"

EXPECTED_BOOKS = [(g, s) for g in range(1, 7) for s in ("上", "下")]
GRADE_CN = {"一": 1, "二": 2, "三": 3, "四": 4, "五": 5, "六": 6}
BOOK_RE = re.compile(r"外研版一年级起点([一二三四五六])年级([上下])\.xlsx$")
...
def parse_source(path: Path, grade: int, semester: str) -> list[dict[str, str]]:
    ...
    for row_no, vals in rows[header_pos + 1:]:
        ...
        key = match_key(word)
        out.append({
            "SourceOccurrenceKey": f"waiyan_start1|g{grade}-{sem_en}|r{row_no:03d}|{key}",
            "SourceID": "waiyan_start1",
            "SourceBook": book,
            "Grade": str(grade),
            "Semester": semester,
...
def main() -> None:
    ...
    for path in SOURCE_DIR.glob("外研版一年级起点*年级*.xlsx"):
        m = BOOK_RE.fullmatch(path.name)
        if m:
            key = (GRADE_CN[m.group(1)], m.group(2))
            if key in source_files:
                raise SystemExit(f"Duplicate Waiyan start1 source book for {key}: {source_files[key].name}, {path.name}")
    ...
    if missing or len(source_files) != 12:
        raise SystemExit(f"Waiyan start1 primary source-book set invalid; missing={missing}; found={len(source_files)}")
    ...
    readme = f"""# Waiyan Start1 — Third-party Source Adapter
...
"""
    ...
    print("Waiyan start1 source adapter = valid")
