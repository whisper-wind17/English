#!/usr/bin/env python3
"""Validate the narrow Waiyan start-from-grade-3 primary source-adapter contract."""
...
PATH = ROOT / "anki" / "klose" / "source_reference" / "waiyan_start3_staging" / "occurrences.csv"
EXPECTED_FIELDS = [
    "SourceOccurrenceKey", "SourceID", "SourceBook", "Grade", "Semester",
    "SourceRow", "Word", "MatchKey", "British", "American", "Definition", "SourceFile",
]
SOURCE_FILE_RE = re.compile(r"外研版三年级起点[三四五六]年级[上下]\.xlsx$")
EXPECTED_OCCURRENCES = 1157
EXPECTED_MATCHKEYS = 1023
...
def main() -> None:
    ...
    if len(rows) != EXPECTED_OCCURRENCES:
        raise SystemExit(f"Expected {EXPECTED_OCCURRENCES} Waiyan start3 occurrences, found {len(rows)}")
    if any(row.get("SourceID") != "waiyan_start3" for row in rows):
        raise SystemExit("SourceID drift in Waiyan start3 adapter")
    ...
    if any(not SOURCE_FILE_RE.fullmatch(row.get("SourceFile", "")) for row in rows):
        raise SystemExit("Unexpected source file leaked into Waiyan start3 primary adapter")
    ...
    print("Waiyan Start3 Source Adapter Recheck = pass")
