#!/usr/bin/env python3
"""Parse Kepu start-from-grade-3 primary vocabulary into standardized occurrences."""
...
SOURCE_DIR = ROOT / "1.全国各大教材版本中小学同步" / "科普版"
OUT = ROOT / "anki" / "klose" / "source_reference" / "kepu_start3_staging"
EXPECTED_BOOKS = [(g, s) for g in range(3, 7) for s in ("上", "下")]
GRADE_CN = {"三": 3, "四": 4, "五": 5, "六": 6}
BOOK_RE = re.compile(r"科普版三年级起点([三四五六])年级([上下])(?:册)?\.xlsx$")
OCC_FIELDS = [
...
def parse_source(path: Path, grade: int, semester: str) -> list[dict[str, str]]:
    ...
    for row_no, vals in rows[header_pos + 1:]:
        ...
        key = match_key(word)
        out.append({
            "SourceOccurrenceKey": f"kepu_start3|g{grade}-{sem_en}|r{row_no:03d}|{key}",
            "SourceID": "kepu_start3", "SourceBook": f"{grade}年级{semester}",
...
def main() -> None:
    ...
    for path in SOURCE_DIR.glob("科普版三年级起点*.xlsx"):
        m = BOOK_RE.fullmatch(path.name)
        if m:
            key = (GRADE_CN[m.group(1)], m.group(2))
            if key in source_files:
                raise SystemExit(f"Duplicate Kepu start3 source book: {key}")
    ...
    if missing or len(source_files) != 8:
        raise SystemExit(f"Kepu start3 source-book set invalid; missing={missing}; found={len(source_files)}")
    ...
    (OUT / "README.md").write_text(
        f"# Kepu Start3 — Third-party Source Adapter\n\n"
        f"Scope: 科普版三年级起点 3-6 年级上下册，共 8 册。\n\n"
    ...
    print("Kepu start3 source adapter = parsed")
