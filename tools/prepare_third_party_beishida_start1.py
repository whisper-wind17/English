#!/usr/bin/env python3
"""Parse Beishida start-from-grade-1 primary vocabulary into standardized occurrences.

Source Adapter responsibility is intentionally narrow:
- parse the 12 Grade 1-6 XLSX books only;
- preserve every source occurrence and raw source fields;
- write occurrences.csv only.

The Beishida start-from-grade-3 sequence and all middle/high-school materials in
...
OUT = ROOT / "anki" / "klose" / "source_reference" / "beishida_start1_staging"

EXPECTED_BOOKS = [(g, s) for g in range(1, 7) for s in ("上", "下")]
GRADE_CN = {"一": 1, "二": 2, "三": 3, "四": 4, "五": 5, "六": 6}
BOOK_RE = re.compile(r"北师大版一年级起点([一二三四五六])年级([上下])(?:册)?\.xlsx$")
...
def parse_source(path: Path, grade: int, semester: str) -> list[dict[str, str]]:
    ...
    for row_no, vals in rows[header_pos + 1:]:
        ...
        key = match_key(word)
        out.append({
            "SourceOccurrenceKey": f"beishida_start1|g{grade}-{sem_en}|r{row_no:03d}|{key}",
            "SourceID": "beishida_start1",
            "SourceBook": book,
            "Grade": str(grade),
            "Semester": semester,
...
def main() -> None:
    ...
    for path in SOURCE_DIR.glob("北师大版一年级起点*.xlsx"):
        ...
        if key in source_files:
            raise SystemExit(
                f"Duplicate Beishida start1 source book for {key}: "
    ...
    if missing or len(source_files) != 12:
        raise SystemExit(
            f"Beishida start1 primary source-book set invalid; missing={missing}; found={len(source_files)}"
        )
    ...
    readme = f"""# Beishida Start1 — Third-party Source Adapter
...
"""
    ...
    print("Beishida start1 source adapter = valid")
