#!/usr/bin/env python3
"""Validate the narrow Minjiao grade-3-start primary source-adapter contract."""
...
def main() -> None:
    ...
    if len(rows) != EXPECTED_OCCURRENCES:
        raise SystemExit(f"Expected {EXPECTED_OCCURRENCES} Minjiao occurrences, found {len(rows)}")
    if any(r.get("SourceID") != "minjiao_start3" for r in rows):
        raise SystemExit("SourceID drift in Minjiao start3 adapter")
    ...
    if any(r.get("British", "") or r.get("American", "") for r in rows):
        raise SystemExit("Minjiao raw source has no pronunciation columns; British/American must remain empty")
    ...
    for r in rows:
        ...
        if not m:
            raise SystemExit(f"Unexpected Minjiao source file: {r.get('SourceFile', '')}")
        ...
        prefix = f"minjiao_start3|g{grade}-{SEM_EN[semester]}|r{row_no:03d}|"
    ...
    if seen_books != expected_books:
        raise SystemExit(f"Expected 8 Minjiao books, got {sorted(seen_books)}")
    ...
    print("Minjiao Start3 Source Adapter Recheck = pass")
