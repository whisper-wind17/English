#!/usr/bin/env python3
"""Parse Xiangshao start-from-grade-3 primary vocabulary into standardized occurrences.
...
SOURCE_DIR = ROOT / "1.全国各大教材版本中小学同步" / "湘少版"
OUT = ROOT / "anki" / "klose" / "source_reference" / "xiangshao_start3_staging"

EXPECTED_BOOKS = [(g, s) for g in range(3, 7) for s in ("上", "下")]
GRADE_CN = {"三": 3, "四": 4, "五": 5, "六": 6}
BOOK_RE = re.compile(r"湘少版三年级起点([三四五六])年级([上下])(?:册)?\.xlsx$")
...
def parse_source(path: Path, grade: int, semester: str) -> list[dict[str, str]]:
    ...
    for row_no, vals in rows[header_pos + 1:]:
        ...
        key = match_key(word)
        out.append({
            "SourceOccurrenceKey": f"xiangshao_start3|g{grade}-{sem_en}|r{row_no:03d}|{key}",
            "SourceID": "xiangshao_start3",
            "SourceBook": book,
            "Grade": str(grade),
            "Semester": semester,
...
def main() -> None:
    ...
    for path in SOURCE_DIR.glob("湘少版三年级起点*.xlsx"):
        ...
        if key in source_files:
            raise SystemExit(
                f"Duplicate Xiangshao start3 source book for {key}: "
    ...
    if missing or len(source_files) != 8:
        raise SystemExit(
            f"Xiangshao start3 primary source-book set invalid; missing={missing}; found={len(source_files)}"
        )
    ...
    readme = f"""# Xiangshao Start3 — Third-party Source Adapter
...
"""
    ...
    print("Xiangshao start3 source adapter = parsed")
