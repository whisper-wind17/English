#!/usr/bin/env python3
"""Validate the narrow Luke 5-4-system grade-3-start primary source-adapter contract."""
...
PATH = ROOT / "anki" / "klose" / "source_reference" / "luke_54_start3_staging" / "occurrences.csv"
EXPECTED_FIELDS = [
    "SourceOccurrenceKey", "SourceID", "SourceBook", "Grade", "Semester",
    "SourceRow", "Word", "MatchKey", "British", "American", "Definition", "SourceFile",
]
SOURCE_FILE_RE = re.compile(r"鲁科版五四学制([三四五])年级([上下])册\.xlsx$")
GRADE_CN = {"三": "3", "四": "4", "五": "5"}
SEM_EN = {"上": "upper", "下": "lower"}
EXPECTED_OCCURRENCES = 714
EXPECTED_MATCHKEYS = 681
EXPECTED_BLANK_DEFINITIONS = 2
...
def main() -> None:
    ...
    if len(rows) != EXPECTED_OCCURRENCES:
        raise SystemExit(f"Expected {EXPECTED_OCCURRENCES} Luke 54 occurrences, found {len(rows)}")
    if any(r.get("SourceID") != "luke_54_start3" for r in rows):
        raise SystemExit("SourceID drift in Luke 54 start3 adapter")
    ...
    if any(r.get("British", "") or r.get("American", "") for r in rows):
        raise SystemExit("Luke 54 raw source has no pronunciation columns; British/American must remain empty")

    blank_defs = [r for r in rows if not r.get("Definition", "").strip()]
    if len(blank_defs) != EXPECTED_BLANK_DEFINITIONS:
        raise SystemExit(
            f"Expected {EXPECTED_BLANK_DEFINITIONS} source blank Definitions, found {len(blank_defs)}"
        )
    ...
    for r in rows:
        ...
        if not m:
            raise SystemExit(f"Unexpected Luke 54 source file: {r.get('SourceFile', '')}")
        grade, semester = GRADE_CN[m.group(1)], m.group(2)
        if grade not in {"3", "4", "5"}:
            raise SystemExit(f"Grade 6+ leaked into Luke 54 primary adapter: {r.get('SourceFile')}")
        ...
        prefix = f"luke_54_start3|g{grade}-{SEM_EN[semester]}|r{row_no:03d}|"
    ...
    expected_books = {(str(g), s) for g in range(3, 6) for s in ("上", "下")}
    if seen_books != expected_books:
        raise SystemExit(f"Expected 6 Luke 54 primary books, got {sorted(seen_books)}")
    ...
    print("Luke 54 Start3 Source Adapter Recheck = pass")
    ...
    print("primary boundary = grades 3-5 only; grade-6-plus leakage = no")
