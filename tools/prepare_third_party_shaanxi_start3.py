#!/usr/bin/env python3
"""Parse Shaanxi grade-3-start primary vocabulary into standardized occurrences."""
...
SOURCE_DIR = ROOT / "1.全国各大教材版本中小学同步" / "陕西版"
OUT = ROOT / "anki" / "klose" / "source_reference" / "shaanxi_start3_staging"
EXPECTED_BOOKS = [(g, s) for g in range(3, 7) for s in ("上", "下")]
GRADE_CN = {"三": 3, "四": 4, "五": 5, "六": 6}
BOOK_RE = re.compile(r"陕西版三年级起点([三四五六])年级([上下])\.xlsx$")
OCC_FIELDS = [
...
def parse_source(path: Path, grade: int, semester: str) -> tuple[list[dict[str, str]], str]:
    ...
    if header_pos is not None:
        ...
        for row_no, vals in rows[header_pos + 1:]:
            ...
            key = match_key(word)
            out.append({
                "SourceOccurrenceKey": f"shaanxi_start3|g{grade}-{sem_en}|r{row_no:03d}|{key}",
                "SourceID": "shaanxi_start3", "SourceBook": f"{grade}年级{semester}",
        ...
        if len(sample) < 3 or any(len(vals) < 2 or not vals[0] for _, vals in sample):
            raise SystemExit(f"Unrecognized Shaanxi XLSX schema in {path.name}; first rows={rows[:10]}")
        if any(any(v for v in vals[2:]) for _, vals in rows):
            raise SystemExit(f"Headerless Shaanxi source has non-empty columns beyond B in {path.name}; first rows={rows[:10]}")
...
def main() -> None:
    ...
    for path in SOURCE_DIR.glob("陕西版三年级起点*.xlsx"):
        m = BOOK_RE.fullmatch(path.name)
        if m:
            key = (GRADE_CN[m.group(1)], m.group(2))
            if key in source_files:
                raise SystemExit(f"Duplicate Shaanxi start3 source book: {key}")
    ...
    if missing or len(source_files) != 8:
        raise SystemExit(f"Shaanxi start3 source-book set invalid; missing={missing}; found={len(source_files)}")
    ...
    (OUT / "README.md").write_text(
        f"# Shaanxi Start3 — Third-party Source Adapter\n\n"
        f"Scope: 陕西版三年级起点 3-6 年级上下册，共 8 册。\n\n"
    ...
    print("Shaanxi start3 source adapter = parsed")
