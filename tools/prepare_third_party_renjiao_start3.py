#!/usr/bin/env python3
"""Parse Renjiao start-from-grade-3 vocabulary into standardized occurrences.

Source Adapter responsibility is intentionally narrow:
- parse the 8 Grade 3-6 XLSX books;
...
OUT = ROOT / "anki" / "klose" / "source_reference" / "renjiao_start3_staging"

EXPECTED_BOOKS = [(g, s) for g in range(3, 7) for s in ("上", "下")]
GRADE_CN = {"三": 3, "四": 4, "五": 5, "六": 6}
BOOK_RE = re.compile(r"人教版三年级起点([三四五六])年级([上下])\.xlsx$")
...
def parse_source(path: Path, grade: int, semester: str) -> list[dict[str, str]]:
    ...
    for row_no, vals in rows[header_pos + 1:]:
        ...
        key = match_key(word)
        out.append({
            "SourceOccurrenceKey": f"renjiao_start3|g{grade}-{sem_en}|r{row_no:03d}|{key}",
...
def main() -> None:
    ...
    for path in SOURCE_DIR.glob("人教版三年级起点*年级*.xlsx"):
    ...
    if missing or len(source_files) != 8:
        raise SystemExit(f"Renjiao start3 source-book set invalid; missing={missing}; found={len(source_files)}")
    ...
    readme = f"""# Renjiao Start3 — Third-party Source Adapter
...
"""
    ...
    print("Renjiao start3 source adapter = valid")
