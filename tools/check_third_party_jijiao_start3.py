#!/usr/bin/env python3
"""Validate the narrow Jijiao start-from-grade-3 primary source-adapter contract."""
...
PATH = ROOT / "anki" / "klose" / "source_reference" / "jijiao_start3_staging" / "occurrences.csv"
EXPECTED_FIELDS = [
...
def main() -> None:
    ...
    if any(not SOURCE_FILE_RE.fullmatch(row.get("SourceFile", "")) for row in rows):
        raise SystemExit("Unexpected source file leaked into Jijiao start3 primary adapter")
    ...
    print("Jijiao Start3 Source Adapter Recheck = pass")
