#!/usr/bin/env python3
"""Parse Minjiao grade-3-start primary vocabulary into standardized occurrences."""
...
SOURCE_DIR = ROOT / "1.全国各大教材版本中小学同步" / "闽教版"
OUT = ROOT / "anki" / "klose" / "source_reference" / "minjiao_start3_staging"
EXPECTED_BOOKS = [(g, s) for g in range(3, 7) for s in ("上", "下")]
GRADE_CN = {"三": 3, "四": 4, "五": 5, "六": 6}
BOOK_RE = re.compile(r"闽教版三年级起点([三四五六])年级([上下])\.xlsx$")
OCC_FIELDS = [
...
def main() -> None:
    ...
    for path in SOURCE_DIR.glob("闽教版三年级起点*.xlsx"):
        m = BOOK_RE.fullmatch(path.name)
        if m:
            key = (GRADE_CN[m.group(1)], m.group(2))
            if key in source_files:
                raise SystemExit(f"Duplicate Minjiao source book: {key}")
    ...
    if missing or len(source_files) != 8:
        raise SystemExit(f"Minjiao source-book set invalid; missing={missing}; found={len(source_files)}")
    ...
    print("Minjiao start3 source adapter = parsed")
