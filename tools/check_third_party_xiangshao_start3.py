#!/usr/bin/env python3
"""Validate the narrow Xiangshao start-from-grade-3 primary source-adapter contract."""
...
PATH = ROOT / "anki" / "klose" / "source_reference" / "xiangshao_start3_staging" / "occurrences.csv"
EXPECTED_FIELDS = [
    "SourceOccurrenceKey", "SourceID", "SourceBook", "Grade", "Semester",
    "SourceRow", "Word", "MatchKey", "British", "American", "Definition", "SourceFile",
]
SOURCE_FILE_RE = re.compile(r"湘少版三年级起点([三四五六])年级([上下])(?:册)?\.xlsx$")
GRADE_CN = {"三": "3", "四": "4", "五": "5", "六": "6"}
SEM_EN = {"上": "upper", "下": "lower"}
EXPECTED_OCCURRENCES = 698
EXPECTED_MATCHKEYS = 674
...
def main() -> None:
    ...
    if len(rows) != EXPECTED_OCCURRENCES:
        raise SystemExit(
            f"Expected {EXPECTED_OCCURRENCES} Xiangshao start3 occurrences, found {len(rows)}"
        )
    if any(row.get("SourceID") != "xiangshao_start3" for row in rows):
        raise SystemExit("SourceID drift in Xiangshao start3 adapter")
    ...
    for row in rows:
        ...
        if not m:
            raise SystemExit(
                f"Unexpected source file leaked into Xiangshao start3 primary adapter: {row.get('SourceFile', '')}"
            )
        ...
        expected_prefix = f"xiangshao_start3|g{grade}-{SEM_EN[semester]}|r{row_no:03d}|"
    ...
    print("Xiangshao Start3 Source Adapter Recheck = pass")
